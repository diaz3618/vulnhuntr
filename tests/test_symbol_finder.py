...
class TestAllNamesSearch:
    def test_fallback_search_finds_symbol(self, extractor, sample_project):
        ...
        files = list((sample_project / "myapp").glob("*.py"))
        scripts = [jedi.Script(path=f, project=extractor.project) for f in files]
...
class TestEdgeCases:
    def test_method_call_on_variable(self, extractor, sample_project):
        """Edge case: calc = Calculator(); calc.add(1, 2)"""
        files = list((sample_project / "myapp").glob("*.py"))
    ...
    def test_aliased_import(self, extractor, sample_project):
        """Edge case: from service import Service as FlowService"""
        files = list((sample_project / "myapp").glob("*.py"))
        result = extractor.extract("Calc", "Calc()", files)
        assert result is not None

    def test_symbol_not_in_any_file(self, extractor, sample_project):
        """Completely unknown symbol returns None through all tiers."""
        files = list((sample_project / "myapp").glob("*.py"))
        result = extractor.extract("zzz_nonexistent", "zzz_nonexistent()", files)
        assert result is None

    def test_empty_file_list_falls_through_to_project(self, extractor):
        """Searching with no files still tries project_search fallback."""
        result = extractor.extract("helper", "helper()", [])
        # The extractor falls through file_search to project/all_names search
        assert result is None or isinstance(result, dict)

    def test_syntax_error_in_file(self, extractor, tmp_path):
        """A file with syntax errors shouldn't crash jedi — returns None or skips."""
        bad = tmp_path / "broken.py"
        bad.write_text("def bad(:\n  pass  # syntax error")
        result = extractor.extract("bad", "bad()", [bad])
        # Should not raise; returns None or some partial result
        assert result is None or isinstance(result, dict)

    def test_dotted_name_resolution(self, extractor, sample_project):
        """Dotted access like Calculator.add should resolve."""
        files = list((sample_project / "myapp").glob("*.py"))
        result = extractor.extract("Calculator.add", "calc.add(1, 2)", files)
        assert result is None or "add" in result.get("source", "")

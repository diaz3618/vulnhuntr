...
class TestReadmeSummary:
    ...
    def test_round_trip(self):
        original, restored = _xml_round_trip(ReadmeSummary, readme_summary="A short summary.")
        assert restored.readme_summary == original.readme_summary


class TestInstructions:
    def test_round_trip(self):
        original, restored = _xml_round_trip(Instructions, instructions="Analyze for SQLI.")
...
class TestResponseFormat:
    def test_round_trip(self):
        ...
        original, restored = _xml_round_trip(ResponseFormat, response_format=schema)
        assert restored.response_format == original.response_format


class TestAnalysisApproach:
    def test_round_trip(self):
        original, restored = _xml_round_trip(AnalysisApproach, analysis_approach="Trace from source to sink.")
        assert restored.analysis_approach == original.analysis_approach


class TestGuidelines:
    def test_round_trip(self):
        original, restored = _xml_round_trip(Guidelines, guidelines="Focus on user-controlled input.")
        assert restored.guidelines == original.guidelines


class TestPreviousAnalysis:
    def test_round_trip(self):
        original, restored = _xml_round_trip(PreviousAnalysis, previous_analysis="Iteration 1 found potential SSRF.")
        assert restored.previous_analysis == original.previous_analysis


class TestExampleBypasses:
    def test_round_trip(self):
        original, restored = _xml_round_trip(ExampleBypasses, example_bypasses="../../../../etc/passwd")
...
class TestCodeDefinitions:
    ...
    def test_with_definitions(self):
        items = [
            CodeDefinition(name="a", context_name_requested="a", file_path="x.py", source="def a(): ..."),
            CodeDefinition(name="b", context_name_requested="b", file_path="y.py", source="def b(): ..."),
        ]
...
class TestMalformedXML:
    """Verify that models reject or handle bad XML gracefully."""

    def test_truncated_xml_raises(self):
        broken = b"<readme_content>no closing tag"
        with pytest.raises(Exception):
            ReadmeContent.from_xml(broken)

    def test_wrong_root_tag(self):
        xml = b"<wrong_tag>content</wrong_tag>"
        with pytest.raises(Exception):
            ReadmeContent.from_xml(xml)

    def test_empty_element_raises_on_required_field(self):
        xml = b"<readme_content></readme_content>"
        with pytest.raises(Exception):
            ReadmeContent.from_xml(xml)

    def test_special_chars_in_content_preserved(self):
        content = "x < y && a > b & c"
        obj = ReadmeContent(content=content)
        xml_bytes = obj.to_xml()
        restored = ReadmeContent.from_xml(xml_bytes)
    ...
    def test_code_definition_with_angle_brackets_in_source(self):
        source = 'if x < 10 and y > 5:\n    return "ok"'
        original, restored = _xml_round_trip(
            CodeDefinition,
            name="check",
            context_name_requested="check",
            file_path="logic.py",
            source=source,
        )

...
class TestResponse:
    ...
    def test_invalid_vuln_type_filtered_not_rejected(self):
        """Unknown vuln types are silently filtered, not rejected (resilience fix)."""
        r = Response(
            scratchpad="s",
            analysis="a",
            confidence_score=1,
        ...
        assert r.vulnerability_types == []

    def test_mixed_valid_and_invalid_vuln_types(self):
        """Valid types kept, invalid types filtered out."""
        r = Response(
            scratchpad="s",
            analysis="a",
            confidence_score=5,
            vulnerability_types=["SSRF", "INFO_DISCLOSURE", "RCE", "AUTH_BYPASS"],
            context_code=[],
        )
        assert r.vulnerability_types == [VulnType.SSRF, VulnType.RCE]

    def test_vuln_type_prefix_stripped(self):
        """LLMs sometimes return 'VulnType.SSRF' — prefix should be stripped."""
        r = Response(
            scratchpad="s",
            analysis="a",
            confidence_score=5,
            vulnerability_types=["VulnType.SSRF", "VulnType.RCE"],
            context_code=[],
        )
        assert r.vulnerability_types == [VulnType.SSRF, VulnType.RCE]

    def test_missing_scratchpad_uses_default(self):
        """After Issue #6 fix, omitted fields use defaults instead of raising."""
        r = Response(
            analysis="a",
            confidence_score=0,
            vulnerability_types=[],

...
class TestValidateResponseEdgeCases:
    """Edge cases for the JSON repair pipeline — backslash-heavy inputs,
    embedded code snippets, and other patterns that trip up naive regex."""

    def test_valid_json_escape_sequences_preserved(self):
        raw = r'{"scratchpad": "line1\nline2\ttab", "value": 3}'
        result = _make_llm()._validate_response(raw, _Stub)
        assert "line1" in result.scratchpad
        assert result.value == 3

    def test_windows_path_backslashes(self):
        raw = '{"scratchpad": "C:\\\\Users\\\\admin\\\\file.py", "value": 5}'
        result = _make_llm()._validate_response(raw, _Stub)
        assert "Users" in result.scratchpad

    def test_regex_pattern_in_field(self):
        raw = '{"scratchpad": "regex: \\\\d+\\\\.\\\\w+", "value": 7}'
        result = _make_llm()._validate_response(raw, _Stub)
        assert result.value == 7

    def test_unicode_escape_preserved(self):
        raw = '{"scratchpad": "emoji \\u2603", "value": 2}'
        result = _make_llm()._validate_response(raw, _Stub)
        assert "\u2603" in result.scratchpad

    def test_deeply_nested_quotes(self):
        raw = '{"scratchpad": "he said \\"hello\\"", "value": 4}'
        result = _make_llm()._validate_response(raw, _Stub)
        assert "hello" in result.scratchpad

    def test_mixed_python_literal_keywords(self):
        raw = '{"scratchpad": "NonEmpty", "value": 0, "flag": True, "x": None, "y": False}'
        result = _make_llm()._validate_response(raw, _Stub)
        # "NonEmpty" must NOT have its "None" prefix replaced
        assert result.scratchpad == "NonEmpty"

    def test_json_with_trailing_comma_still_fails(self):
        raw = '{"scratchpad": "ok", "value": 1,}'
        with pytest.raises(LLMError):
            _make_llm()._validate_response(raw, _Stub)

    def test_multiple_json_objects_greedy_match(self):
        r"""The regex \{.*\} is greedy/DOTALL, so it captures from first { to last }."""
        first = '{"scratchpad": "first", "value": 1}'
        raw = f"Some text {first} extra trailing"
        result = _make_llm()._validate_response(raw, _Stub)
        assert result.scratchpad == "first"

    def test_extremely_long_value_parses(self):
        long_text = "a" * 10000
        raw = f'{{"scratchpad": "{long_text}", "value": 9}}'
        result = _make_llm()._validate_response(raw, _Stub)
        assert len(result.scratchpad) == 10000
...
class TestValidateResponseFailure:
    ...
    def test_raises_on_empty_response(self):
        with pytest.raises(LLMError, match="empty response"):
            _make_llm()._validate_response("", _Stub)

    def test_raises_on_whitespace_only_response(self):
        with pytest.raises(LLMError, match="empty response"):
            _make_llm()._validate_response("   \n  ", _Stub)
...
class TestClaudeMessages:
    ...
    def test_readme_prompt_has_no_prefill(self, mock_cls):
        ...
        msgs = c.create_messages("Provide a very concise summary of the README.md content below")
        assert len(msgs) == 1
    ...
    @patch.dict("os.environ", {"ANTHROPIC_API_KEY": "test-key"})
    @patch("vulnhuntr.llms.anthropic.Anthropic")
    def test_get_response_preserves_newlines(self, mock_cls):
        c = Claude(model="m", base_url="https://api.anthropic.com")
        fake_resp = SimpleNamespace(content=[SimpleNamespace(text="line1\nline2\nline3")])
        assert c.get_response(fake_resp) == "line1\nline2\nline3"
...
class TestClaudeErrors:
    ...
    def test_connection_error_wrapped(self, mock_cls):
        ...
        c = Claude(model="m", base_url="https://api.anthropic.com")
        c.client.messages.create.side_effect = anth.APIConnectionError(request=MagicMock())
...
class TestClaudeUsage:
    ...
    def test_extract_usage(self, mock_cls):
        c = Claude(model="claude-test", base_url="https://api.anthropic.com")
        resp = SimpleNamespace(usage=SimpleNamespace(input_tokens=200, output_tokens=80))
...
class TestChatGPTMessages:
    ...
    def test_json_mode_when_response_model(self, mock_cls):
        ...
        g.client.chat.completions.create = MagicMock(
            return_value=SimpleNamespace(
                choices=[SimpleNamespace(message=SimpleNamespace(content='{"scratchpad":"x","value":1}'))],
    ...
    def test_no_json_mode_without_response_model(self, mock_cls):
        ...
        g.client.chat.completions.create = MagicMock(
            return_value=SimpleNamespace(
                choices=[SimpleNamespace(message=SimpleNamespace(content="just text"))],
                usage=SimpleNamespace(prompt_tokens=10, completion_tokens=5),
            )
        )
...
class TestChatGPTErrors:
    ...
    def test_connection_error_wrapped(self, mock_cls):
        ...
        g.client.chat.completions.create.side_effect = oai.APIConnectionError(request=MagicMock())
        with pytest.raises(APIConnectionError):
...
class TestRateLimitRetry:
    """Verify that ChatGPT and OpenRouter retry with backoff on rate limits."""

    @patch("vulnhuntr.llms.time.sleep")
    @patch("vulnhuntr.llms.openai.OpenAI")
    def test_chatgpt_retries_on_rate_limit(self, mock_openai_cls, mock_sleep):
        ...
        mock_client.chat.completions.create.side_effect = [rate_err, rate_err, success_response]

        gpt = ChatGPT(model="gpt-4o", base_url="https://api.openai.com/v1")
        result = gpt.send_message([{"role": "user", "content": "hi"}], 100, _max_retries=3, _base_delay=0.01)
    ...
    @patch("vulnhuntr.llms.time.sleep")
    @patch("vulnhuntr.llms.openai.OpenAI")
    def test_chatgpt_raises_after_max_retries(self, mock_openai_cls, mock_sleep):
        from unittest.mock import PropertyMock
        ...
        with pytest.raises(RateLimitError, match="rate-limited after 3 retries"):
            gpt.send_message([{"role": "user", "content": "hi"}], 100, _max_retries=3, _base_delay=0.01)

    @patch("vulnhuntr.llms.time.sleep")
    @patch("vulnhuntr.llms.openai.OpenAI")
    def test_openrouter_retries_on_rate_limit(self, mock_openai_cls, mock_sleep):
        from unittest.mock import PropertyMock
        ...
        mock_client.chat.completions.create.side_effect = [rate_err, success_response]

        with patch.dict("os.environ", {"OPENROUTER_API_KEY": "test-key"}):
            orr = OpenRouter(model="meta-llama/llama-3.3-70b:free", base_url="https://openrouter.ai/api/v1")
        result = orr.send_message([{"role": "user", "content": "hi"}], 100, _max_retries=3, _base_delay=0.01)
    ...
    @patch("vulnhuntr.llms.time.sleep")
    @patch("vulnhuntr.llms.openai.OpenAI")
    def test_openrouter_raises_after_max_retries(self, mock_openai_cls, mock_sleep):
        from unittest.mock import PropertyMock

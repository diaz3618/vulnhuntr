...
class MarkdownReporter(ReporterBase):
    ...
    def generate(self) -> str:
        ...
        lines = [
            f"# {self.title}",
            "",
            f"> Generated by **Vulnhuntr** v{self.metadata.get('tool_version', '1.0.0')} on {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}",
            "",
        ]
        ...
        lines.extend(
            [
                "---",
                "",
                "*This report was automatically generated by [Vulnhuntr](https://github.com/diaz3618/vulnhuntr), an LLM-powered vulnerability scanner.*",
            ]
        )
